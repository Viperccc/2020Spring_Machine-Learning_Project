...
from load_cifar10 import  load_dataset_cifar10
from load_svhn import load_dataset_svhn
...
#x_train,y_train,x_va,y_va,x_te,y_te = load_dataset_cifar10(va_ratio=0.5)
...
new_a = np.delete(a,drop_index)
print(max(new_a))
sb_x_train = x_train[new_a]
sb_y_train1 = y_train.copy()
sb_y_train = y_train[new_a]
...
# baseline: random correctrion
u_idxs = np.arange(x_train.shape[0])
random_size = length
uniform_idx = np.random.choice(u_idxs,random_size,replace=False)
us_x_train = x_train[u_idxs]
us_y_train = y_train[u_idxs]
us_y_train[uniform_idx] = np.logical_xor(np.ones(random_size), us_y_train[uniform_idx]).astype(int)
...
print("[Dropout]  logloss {:.6f}, # {}".format(sb_logloss,sb_size))
...
print("[Dropout]  logloss {:.6f}, # {}".format(sb_te_logloss,sb_size))
